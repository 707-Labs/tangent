...
| `/graph` | Zoom-out playground (prototype): the current article centered with its live candidate pool fanned into directional sectors (wild / place / deeper / theme / era), a tappable era-place "zoom out" ladder, and a re-rootable trail. Motion: center and satellites share one keyed list, so a tapped node glides to the middle (500ms ease-out), shared nodes glide between sectors, new ones fly out from the center with staggered delays, edges grow in. Seeds hydrate categories via `/api/card?categories=1` so the first hop sectors immediately. Compact stages (<480px) show two per sector on a single vertically stretched ring and hide the radial sector labels. Client-only sandbox for the ambient-minimap idea — not linked from the feed yet. |
